...
복제 자료에는 Gemini Canvas용 제작 요청, 정적 앱 소스, 기본 `USAGE.md`, `marked`와 `DOMPurify`가 함께 들어갑니다. Google Fonts는 시스템 글꼴 스택으로 바뀌며, 원본 사이트나 CDN을 다시 요청하지 않아도 실행됩니다. 화면 DOM을 캡처하거나 `localStorage`를 읽지 않으므로 사용자가 작성한 메모, 제목, 화면 설정은 포함되지 않습니다.
