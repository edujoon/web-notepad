...
마크다운이 섞인 글을 세 가지 방식으로 보고 편집하고 복사할 수 있는 정적 웹 메모장입니다. 이 앱을 체험한 사람이 실제 소스와 제작 요청문을 Gemini로 가져가, 별도 설치나 배포 없이 Gemini Canvas에서 같은 기능의 앱을 바로 만들도록 구성했습니다. 원본 단일 HTML 앱은 일반 브라우저와 Vercel에서도 계속 독립적으로 실행됩니다.
...
- **앱 복제**에서 Gemini Canvas용 자체 포함 HTML과 제작 요청문 다운로드·복사

## 앱 복제

화면 오른쪽 위의 **앱 복제**를 누르면 별도 설치나 배포 없이 Gemini Canvas에서 조작할 수 있는 앱 미리보기를 요청할 수 있습니다. 복제 자료가 길기 때문에 파일 다운로드와 첨부를 우선 안내하고, 클립보드 복사는 보조 방법으로 제공합니다.

1. **복제 파일 다운로드**로 UTF-8 텍스트 파일을 받습니다.
2. Gemini에서 새 대화를 열고 **Canvas**를 선택합니다.
3. 다운로드한 파일을 첨부합니다.
4. `첨부 파일의 제작 요청과 전체 소스를 읽고, 추가 질문 없이 Gemini Canvas에서 바로 사용할 수 있는 메모장 앱을 만들어 줘.`라고 입력해 전송합니다.
5. Canvas 미리보기에서 메모장을 사용합니다.

**Gemini 열기**는 [gemini.google.com/app](https://gemini.google.com/app)을 새 탭으로 열기만 합니다. Canvas 선택과 파일 첨부는 사용자가 직접 해야 하며, 자동 입력 URL이나 비공식 연결 방식은 사용하지 않습니다. 파일 첨부가 어려울 때는 보조 기능인 **코드 + 프롬프트 복사**로 같은 자료를 붙여 넣을 수 있습니다.

복제 자료에는 Gemini Canvas용 제작 요청, 정적 앱 소스, 복제본용으로 정리한 기본 사용 가이드, `marked`와 `DOMPurify`가 함께 들어갑니다. Google Fonts는 시스템 글꼴 스택으로 바뀌며, 원본 사이트나 CDN을 다시 요청하지 않아도 실행됩니다. 화면 DOM을 캡처하거나 `localStorage`를 읽지 않으므로 사용자가 작성한 메모, 제목, 화면 설정은 포함되지 않습니다.

복제본은 메모장 핵심 기능만 담은 최종 실행 HTML입니다. 원본에만 있는 **앱 복제** 버튼·안내 창·Gemini 연결·복제 생성 코드·재복제용 템플릿은 포함하지 않습니다. 복제본 사용 가이드에서도 앱 복제 설명을 제거하며, 원본 제작자의 이메일·연락처·별도 소개·최초 작성 날짜는 복제 자료에 넣지 않습니다. 프로젝트 저작권은 `Copyright (c) 2026 JOON`으로 유지합니다.

브라우저 저장소가 차단된 미리보기에서는 메모를 현재 세션의 메모리에만 유지하고, 상태 표시줄에 창을 닫으면 사라진다고 표시합니다. 클립보드나 다운로드가 차단되면 해당 동작만 실패 안내를 보여 주고 편집·탭·미리보기는 계속 작동합니다.
...
앱 본체는 HTML·CSS·JavaScript가 한 파일에 들어 있는 정적 페이지입니다. React나 Vite는 필요하지 않습니다. 소스 또는 `USAGE.md`를 수정한 뒤에는 다음 명령으로 원본 안내문과, 원본 전용 복제 기능을 제거한 자체 포함 복제 HTML을 배포용 파일에 갱신합니다.
...
일반적인 브라우저 동작 검증에는 `file://`보다 정적 서버를 권장합니다. 빌드된 복제본은 첫 사용 가이드와 필요한 라이브러리를 내부에 포함하므로, 실행할 때 현재 주소나 원본 사이트의 HTML을 다시 불러오지 않습니다.
...
- Gemini Canvas 제공 여부와 브라우저 권한은 계정·클라이언트·실행 환경에 따라 달라질 수 있습니다. 지원되지 않는 환경에서는 프롬프트가 성공을 가장하지 않고 제한과 필요한 사용자 조작을 밝히도록 작성되어 있습니다.
