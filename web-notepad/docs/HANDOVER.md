...
체험판이 배포용보다 큰 이유는, 웹 복제 때 체험판 초기화 코드를 제외한 배포용 원본을 건네기 위해 원본 전체를 문자열로 품고 있기 때문이에요. 배포용과 체험판 모두 같은 **웹 복제** UI를 사용합니다.
...
- **복제 원본 지정**: 공통 웹 복제 UI가 체험판 자체가 아니라 배포용 `notepad.html`과 `USAGE.md`를 사용하도록 원본을 지정해요.
...
1. **배포용을 고쳤으면 체험판을 다시 만들어야 해요.** `tools/build_demo.py`를 실행하면 `notepad.html`과 `USAGE.md`를 읽어 체험판을 새로 만들어요. 이 단계를 빠뜨리면 체험판의 기능과 웹 복제로 가져가는 원본이 옛 버전이 돼요.
...
- `loadDefaultUsage`, `buildCloneKit`, `openClone` — 기본 가이드를 단일 HTML에 넣고 제작 요청문과 함께 복사·다운로드하는 웹 복제 기능이에요. 현재 DOM이나 localStorage는 읽지 않아요.
...
| `clone_test.py` | 웹 복제 자료의 프롬프트·전체 HTML·개인 메모 제외·재복제 | 전 항목 통과 |
