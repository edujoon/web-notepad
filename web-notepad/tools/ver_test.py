...
async def main():
    async with async_playwright() as p:
        ...
        ok('웹 메모장 사용 가이드' in v, '새 안내문으로 시작', f'{len(v)}자')
        meta = await A.evaluate("() => JSON.parse(localStorage.getItem('__mockdb'))['meta/demo']")
        ok(bool(meta.get('usageVersion')), '안내문 판 번호 저장', meta.get('usageVersion'))
        # 예전 안내문이 남아 있고 초기화 시각이 방금인 상태를 그대로 흉내내기
        await A.evaluate("""() => { const all = JSON.parse(localStorage.getItem('__mockdb'));
          all['notes/demo-home'].body = '# 예전 안내문'; all['meta/demo'] = { lastReset: Date.now(), usageVersion: 'old12345', by: 'x' };
        ...
        ok('웹 메모장 사용 가이드' in v, '안내문이 바뀌면 다음 주기를 기다리지 않고 바로 새 글로', f'{len(v)}자')
        await b.close()
asyncio.run(main())
