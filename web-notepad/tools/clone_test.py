...
async def main():
    async with async_playwright() as p:
        ...
        ok('내가 작성한 메모는 포함되지 않아요' in text and all(step in text for step in ['코드와 프롬프트 복사 또는 파일 다운로드', 'ChatGPT에 붙여넣기 또는 첨부', '앱 미리보기 생성 요청']), '앱 복제 안내 창의 개인정보 안내와 미리보기 순서')
        ...
        ok('최우선 결과물은 ChatGPT 안의 Canvas 또는 HTML·React 미리보기' in bundle and '앱 이름, 사용자 표시 이름, 대표 색상은 묻지 말고' in bundle, '사전 질문 없는 미리보기 우선 프롬프트 포함')
        ok('Vercel에 배포하는 방법' not in bundle and '로컬 실행 방법' not in bundle, '기본 결과물의 파일·Vercel 안내 지시 제거')
        ...
        ok('최우선 결과물은 ChatGPT 안의 Canvas 또는 HTML·React 미리보기' in again['data'] and 'cdn.jsdelivr.net' not in app_html(again['data']), '재복제 자료에도 미리보기 프롬프트와 자체 포함 소스 유지')
